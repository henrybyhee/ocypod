...
use ocypod::models::{job, queue, Duration, OcyError, ServerInfo};
...
impl QueueWrapper {
    ...
    async fn queue_size(&self, conn: &mut Connection) -> u64 {
        self.redis_manager
            .queue_size(conn, &self.queue_name)
            .await
            .unwrap()
    }
    ...
    async fn create_queue(&self, conn: &mut Connection) {
        ...
        assert!(self
            .redis_manager
            .create_or_update_queue(conn, &self.queue_name, &queue::Settings::default())
            .await
            .unwrap());
    }
    ...
    async fn new_job(&self, conn: &mut Connection, job_req: &job::CreateRequest) -> job::JobMeta {
        let job_id = self
            .redis_manager
            .create_job(conn, &self.queue_name, job_req)
            .await
            .unwrap();
    ...
    async fn new_running_default_job(&self, conn: &mut Connection) -> job::Payload {
        self.new_running_job(conn, &job::CreateRequest::default())
            .await
    }
    ...
    async fn job_fields(
        ...
        self.redis_manager
            .job_fields(conn, job_id, Some(fields))
            .await
            .unwrap()
    }

    async fn fail_job(&self, conn: &mut Connection, job_id: u64) -> job::JobMeta {
        let update_req = job::UpdateRequest {
            status: Some(job::Status::Failed),
            output: None,
        };
        self.redis_manager
            .update_job(conn, job_id, &update_req)
            .await
            .unwrap();
        let job_info = self.job_meta(conn, job_id).await;
    ...
    async fn complete_job(&self, conn: &mut Connection, job_id: u64) -> job::JobMeta {
        let update_req = job::UpdateRequest {
            status: Some(job::Status::Completed),
            output: None,
        };
        self.redis_manager
            .update_job(conn, job_id, &update_req)
    ...
    async fn next_job(&self, conn: &mut Connection) -> job::Payload {
        let job_payload = self
            .redis_manager
            .next_queued_job(conn, &self.queue_name)
            .await
            .unwrap()
            .unwrap();
...
async fn queue_create_delete() {
    ...
    assert_eq!(
        redis_manager
            .delete_queue(&mut conn, DEFAULT_QUEUE)
            .await
            .unwrap(),
        false
    );
    assert_eq!(
        redis_manager
            .create_or_update_queue(&mut conn, DEFAULT_QUEUE, &queue_settings)
            .await
            .unwrap(),
        true
    );
...
async fn queue_delete_jobs() {
    ...
    qw.redis_manager
        .update_job(&mut conn, job_payload.id(), &update_req)
        .await
        .unwrap();
    ...
    let job_payload = qw.next_job(&mut conn).await;
    let update_req = job::UpdateRequest {
        status: Some(job::Status::Cancelled),
        output: None,
    };
    qw.redis_manager
        .update_job(&mut conn, job_payload.id(), &update_req)
    ...
    for job_id in 5..10 {
        assert_eq!(
            qw.redis_manager.job_status(&mut conn, job_id).await,
...
async fn queue_names() {
    ...
    let queue_settings = queue::Settings::default();
    assert_eq!(
        redis_manager.queue_names(&mut conn).await.unwrap(),
        Vec::<String>::new()
    );
    assert_eq!(
        redis_manager
            .create_or_update_queue(&mut conn, "d", &queue_settings)
            .await
            .unwrap(),
        true
    );
    assert_eq!(
        redis_manager
            .create_or_update_queue(&mut conn, "b", &queue_settings)
            .await
            .unwrap(),
        true
    );
    assert_eq!(
        redis_manager
            .create_or_update_queue(&mut conn, "a", &queue_settings)
            .await
            .unwrap(),
        true
    );
    assert_eq!(
        redis_manager
            .create_or_update_queue(&mut conn, "c", &queue_settings)
            .await
            .unwrap(),
        true
    );
...
async fn queue_settings() {
    ...
    assert_eq!(
        redis_manager.queue_settings(&mut conn, queue_name).await,
        Err(OcyError::NoSuchQueue(queue_name.to_owned()))
    );
    assert_eq!(
        redis_manager
            .create_or_update_queue(&mut conn, queue_name, &settings)
            .await,
        Ok(true)
    );
    assert_eq!(
        redis_manager
            .queue_settings(&mut conn, queue_name)
            .await
            .unwrap(),
        settings
    );
...
async fn job_not_exists() {
    let (_ctx, mut conn, redis_manager) = init().await;
    assert_eq!(
        redis_manager.job_fields(&mut conn, 1, None).await,
        Err(OcyError::NoSuchJob(1))
    );
    assert_eq!(
        redis_manager.job_status(&mut conn, 1).await,
        Err(OcyError::NoSuchJob(1))
    );
}
...
async fn next_job() {
    ...
    // Push jobs
    let first_job_req = job::CreateRequest {
        input: Some("first_job".into()),
        ..Default::default()
    };
    let first_job = qw.new_job(&mut conn, &first_job_req).await;

    let second_job_req = job::CreateRequest {
        input: Some("second_job".into()),
        ..Default::default()
    };
    let _ = qw.new_job(&mut conn, &second_job_req).await;
    let jobs_count = qw.queue_size(&mut conn).await;
    assert_eq!(jobs_count, 2);

    let job_retrieved = qw.next_job(&mut conn).await;
    assert_eq!(job_retrieved.id(), first_job.id());
...
async fn job_output() {
    let (_ctx, mut conn, redis_manager) = init().await;
    assert_eq!(
        redis_manager.job_output(&mut conn, 123).await,
        Err(OcyError::NoSuchJob(123))
    );
    // TODO: populate test
}
...
async fn job_deletion() {
    ...
    assert_eq!(
        redis_manager.running_queue_size(&mut conn).await.unwrap(),
        1
    );
    assert!(redis_manager
        .tagged_job_ids(&mut conn, "tag")
        .await
        .unwrap()
        .contains(&job_id_running));
    assert!(redis_manager
        .delete_job(&mut conn, job_id_running)
        .await
        .unwrap());
    assert_eq!(
        redis_manager.running_queue_size(&mut conn,).await.unwrap(),
    ...
    assert!(redis_manager
        ...
        .await
        .unwrap()
        .contains(&job_id_completed));
    assert!(redis_manager
        .delete_job(&mut conn, job_id_completed)
        .await
        .unwrap());
    assert!(!redis_manager
        .tagged_job_ids(&mut conn, "tag")
    ...
    assert_eq!(
        redis_manager.failed_queue_size(&mut conn,).await.unwrap(),
        2
    );
    assert!(redis_manager
        .tagged_job_ids(&mut conn, "tag")
        .await
        .unwrap()
        .contains(&job_id_failed));
    assert!(redis_manager
        .delete_job(&mut conn, job_id_failed)
        .await
        .unwrap());
    ...
    assert!(redis_manager
        ...
        .await
        .unwrap()
        .contains(&job_id_cancelled));
    assert!(redis_manager
        .delete_job(&mut conn, job_id_cancelled)
        .await
        .unwrap());
    assert!(!redis_manager
        .tagged_job_ids(&mut conn, "tag")
    ...
    assert!(redis_manager
        ...
        .await
        .unwrap()
        .contains(&job_id_timed_out));
    assert!(redis_manager
        .delete_job(&mut conn, job_id_timed_out)
        .await
        .unwrap());
    ...
    assert_eq!(
        redis_manager
            .queue_size(&mut conn, DEFAULT_QUEUE)
            .await
            .unwrap(),
        1
    );
    assert!(redis_manager
        ...
        .await
        .unwrap()
        .contains(&job_id_queued));
    assert!(redis_manager
        .delete_job(&mut conn, job_id_queued)
        .await
        .unwrap());
    assert_eq!(
        redis_manager
...
async fn job_retry_no_queue() {
    ...
    // 1st retry
    assert_eq!(
        redis_manager.check_job_retries(&mut conn).await.unwrap(),
        Vec::<u64>::new()
    );
    let job_info = qw.job_meta(&mut conn, job_id).await;
    assert_eq!(job_info.status(), job::Status::Failed);
...
async fn job_retries() {
    ...
    assert_eq!(
        redis_manager.check_job_timeouts(&mut conn).await.unwrap(),
        vec![job_id]
    );
    assert_eq!(
        qw.job_status(&mut conn, job_id).await,
        job::Status::TimedOut
    );
...
async fn tag_creation() {
    ...
    assert_eq!(
        qw.job_meta(&mut conn, job_id).await.tags(),
        Some(tags.clone())
    );
    ...
    assert_eq!(
        redis_manager.tagged_job_ids(&mut conn, "some_tag").await,
        Ok(Vec::new())
    );
    ...
    for tag in &tags {
        assert_eq!(
            redis_manager.tagged_job_ids(&mut conn, tag).await,
            Ok(vec![job_id])
        );
    }
    ...
    foo_tagged.sort();
    assert_eq!(
        redis_manager.tagged_job_ids(&mut conn, "foo").await,
        Ok(foo_tagged)
    );
}
...
async fn tag_deletion() {
    ...
    for _ in 0..4 {
        ...
        assert_eq!(
            redis_manager
                .job_fields(&mut conn, job_id, None)
                .await
                .unwrap()
                .tags(),
            Some(tags.clone())
    ...
    for tag in &tags {
        assert_eq!(
            redis_manager.tagged_job_ids(&mut conn, tag).await.unwrap(),
            tagged_ids
        );
    }
...
async fn job_fields() {
    ...
    // single fields
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::Id])
            .await
            .id(),
        1
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::Queue])
            .await
            .queue(),
        DEFAULT_QUEUE
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::Status])
            .await
            .status(),
        job::Status::Queued
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::Tags])
            .await
            .tags(),
        None
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::CreatedAt])
            .await
            .created_at(),
        job_meta.created_at()
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::StartedAt])
            .await
            .started_at(),
        None
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::EndedAt])
            .await
            .ended_at(),
        None
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::LastHeartbeat])
            .await
            .last_heartbeat(),
        None
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::Input])
            .await
            .input(),
        None
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::Output])
            .await
            .output(),
        None
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::Timeout])
            .await
            .timeout(),
        job_meta.timeout()
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::HeartbeatTimeout])
            .await
            .heartbeat_timeout(),
        job_meta.heartbeat_timeout()
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::ExpiresAfter])
            .await
            .expires_after(),
        job_meta.expires_after()
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::Retries])
            .await
            .retries(),
        job_meta.retries()
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::RetriesAttempted])
            .await
            .retries_attempted(),
        0
    );
    assert_eq!(
        qw.job_fields(&mut conn, job_id, &[job::Field::RetryDelays])
            .await
            .retry_delays(),
        None
    );
    ...
    // multiple fields
    let jm = qw
        .job_fields(&mut conn, job_id, &[job::Field::Status, job::Field::Output])
        .await;
    ...
    let jm = qw
        .job_fields(
            &mut conn,
            job_id,
            &[job::Field::Id, job::Field::Tags, job::Field::RetryDelays],
        )
        .await;
...
async fn update_job_heartbeat() {
    ...
    let job_id = qw.next_job(&mut conn).await.id();
    assert!(qw
        .job_meta(&mut conn, job_id)
        .await
        .last_heartbeat()
        .is_none());

    redis_manager
        .update_job_heartbeat(&mut conn, job_id)
        .await
        .unwrap();
    let hb1 = qw
        .job_meta(&mut conn, job_id)
...
async fn update_job_output() {
    ...
    match redis_manager
        .set_job_output(&mut conn, 21, &"foo".into())
        .await
    {
    ...
    match redis_manager
        .set_job_output(&mut conn, job_id, &"foo".into())
        .await
    {
    ...
    redis_manager
        ...
        .unwrap();
    assert_eq!(
        qw.job_meta(&mut conn, job_id).await.output(),
        Some("foo".into())
    );
    ...
    redis_manager
        .set_job_output(&mut conn, job_id, &map)
        .await
        .unwrap();
...
async fn queued_status_transitions() {
    ...
    for new_status in not_allowed {
        match redis_manager
            .set_job_status(&mut conn, job_id, new_status)
            .await
        {
            Err(OcyError::Conflict(_)) => (),
            x => assert!(
                false,
                "Unexpected result when changing status Queued -> {}: {:?}",
                new_status, x
            ),
        }
    }
    ...
    redis_manager
        .set_job_status(&mut conn, job_id, &job::Status::Cancelled)
        .await
        .unwrap();
    let job_info = qw.job_meta(&mut conn, job_id).await;
...
async fn running_status_transitions() {
    ...
    for new_status in not_allowed {
            ...
            .await
        {
            ...
            x => assert!(
                false,
                "Unexpected result when changing status Running -> {}: {:?}",
                new_status, x
            ),
        }
    }
...
async fn completed_status_transitions() {
    ...
    for new_status in not_allowed {
            ...
            .await
        {
            ...
            x => assert!(
                false,
                "Unexpected result when changing status Completed -> {}: {:?}",
                new_status, x
            ),
        }
    }
...
async fn job_heartbeat_timeout() {
    ...
    assert_eq!(
        qw.job_status(&mut conn, job_id_a).await,
        job::Status::Running
    );
    assert_eq!(
        qw.job_status(&mut conn, job_id_b).await,
        job::Status::Running
    );
    assert_eq!(
        qw.job_status(&mut conn, job_id_c).await,
        job::Status::Running
    );
    ...
    redis_manager
        .update_job_heartbeat(&mut conn, job_id_c)
        .await
        .unwrap();
    tokio::time::sleep(time::Duration::from_secs(2)).await;
    assert_eq!(
    ...
    assert_eq!(
        qw.job_meta(&mut conn, job_id_a).await.last_heartbeat(),
        None
    );
    assert_eq!(
        qw.job_meta(&mut conn, job_id_b).await.last_heartbeat(),
        None
    );
    assert!(qw
        .job_meta(&mut conn, job_id_c)
        .await
        .last_heartbeat()
        .is_some());
}
...
async fn job_expiry() {
    ...
    assert_eq!(
        redis_manager.job_status(&mut conn, job_id_running).await,
        Ok(job::Status::Running)
    );
    assert_eq!(
        redis_manager.job_status(&mut conn, job_id_completed).await,
        Err(OcyError::NoSuchJob(job_id_completed))
    );
    assert_eq!(
        redis_manager.job_status(&mut conn, job_id_failed).await,
        Err(OcyError::NoSuchJob(job_id_failed))
    );
    assert_eq!(
        redis_manager.job_status(&mut conn, job_id_cancelled).await,
        Err(OcyError::NoSuchJob(job_id_cancelled))
    );
    assert_eq!(
        redis_manager.job_status(&mut conn, job_id_timed_out).await,
        Err(OcyError::NoSuchJob(job_id_timed_out))
    );
    assert_eq!(
        redis_manager.job_status(&mut conn, job_id_queued).await,
...
async fn create_job_in_all_states(
    ...
    let job_id_running = redis_manager
        .create_job(conn, queue, &job_req)
        .await
        .unwrap();
    let job_id_completed = redis_manager
        .create_job(conn, queue, &job_req)
        .await
        .unwrap();
    let job_id_failed = redis_manager
        .create_job(conn, queue, &job_req)
        .await
        .unwrap();
    let job_id_cancelled = redis_manager
        .create_job(conn, queue, &job_req)
        .await
        .unwrap();

    job_req.timeout = Some(Duration::from_secs(1));
    let job_id_timed_out = redis_manager
        .create_job(conn, queue, &job_req)
        .await
        .unwrap();
    let job_id_queued = redis_manager
        .create_job(conn, queue, &job_req)
        .await
        .unwrap();
    ...
    redis_manager
        .update_job(conn, job_id_completed, &update_req)
        .await
        .unwrap();
    ...
    redis_manager
        .update_job(conn, job_id_failed, &update_req)
        .await
        .unwrap();
    ...
    redis_manager
        .update_job(conn, job_id_cancelled, &update_req)
        .await
        .unwrap();
    ...
    tokio::time::sleep(time::Duration::from_secs(2)).await;
    assert_eq!(
        redis_manager.check_job_timeouts(conn).await.unwrap(),
        vec![job_id_timed_out]
    );

    assert_eq!(
        redis_manager.job_status(conn, job_id_running).await,
        Ok(job::Status::Running)
    );
    assert_eq!(
        redis_manager.job_status(conn, job_id_completed).await,
        Ok(job::Status::Completed)
    );
    assert_eq!(
        redis_manager.job_status(conn, job_id_failed).await,
        Ok(job::Status::Failed)
    );
    assert_eq!(
        redis_manager.job_status(conn, job_id_cancelled).await,
        Ok(job::Status::Cancelled)
    );
    assert_eq!(
        redis_manager.job_status(conn, job_id_timed_out).await,
        Ok(job::Status::TimedOut)
    );
    assert_eq!(
        redis_manager.job_status(conn, job_id_queued).await,
        Ok(job::Status::Queued)
    );
